...
- **Backend & Automation**:
  ...
  - The system logs the event in the `sos_log` table inside `guardianbridge.db` (SQLite) for after-action review.
  ...
  - A user's active SOS status is recorded in the `node_status` table inside `guardianbridge.db`. This status persists even if the node goes offline, ensuring responders are aware of the last known state.
...
* **Queuing Failed Messages:** When the dispatcher fails to send a message to a specific destination, it does not discard it. Instead, it saves the message to a persistent queue in the SQLite `failed_dm_queue` table (legacy JSON is migrated on first run and treated as read-only).
...
* **Actions Tab**: Allows manual tasks like forcing an immediate weather fetch, processing emails, clearing the outgoing email queue, and inspecting/exporting the outgoing email quarantine. It features an "SOS Alert Log" which displays a full history of all received alerts. Administrators can also remotely clear a user's active SOS alert using the "Admin Clear SOS" button, which sends a "STAND DOWN" message to all relevant responders. A new section allows for managing a **Blocked Email Senders** list to prevent spam.
...
* **Settings Tab**: Allows for easy editing of the system's core `.env` configuration file and provides SQLite maintenance actions (integrity check, WAL checkpoint, vacuum). VACUUM attempts to stop and restart the dispatcher via `systemctl`, so ensure the web server user has sudo permissions or run during maintenance. This is for things like GPS coordinates, email credentials, broadcast intervals, weather data staleness (e.g., `WEATHER_DATA_MAX_AGE_MINUTES`), and the outgoing email quarantine retention limit. **A dispatcher restart is required for these changes to take effect**.
...
| `tagin/TAGNAME` | Temporarily join a tag channel to send messages to that group. Ex: `tagin/CERT`. |
| `tagout` | Exit the tag channel and return to normal messaging. |
...
* **A user is blocked/unblocked, but it doesn't take effect**: Restart the dispatcher service (<code>sudo systemctl restart guardianbridge.service</code>) to force it to reload subscribers from `guardianbridge.db`.
...
├── outgoing\_emails.json \# Legacy queue (migrated to SQLite; read-only)
├── failed\_dm\_queue.json \# Legacy queue (migrated to SQLite; read-only)
...
Legacy queue/schedule files remain read-only by default. To remove them after migration, run:
```bash
...
1.  **Validate systemd restart behavior** to confirm the gateway will auto-recover on crashes:
    ...
    You should see the service return to `active` within the RestartSec window (default: 10s).

2.  **Add a dispatcher health check** using the built-in script (optional auto-restart):
    ...
    ```bash
    */5 * * * * /usr/bin/python3 /opt/GuardianBridge/scripts/healthcheck_guardianbridge.py --max-age-seconds 120 --restart >> /opt/GuardianBridge/data/healthcheck.log 2>&1
    ```

3.  **Set up backups** for `guardianbridge.db` and `data/`:

    ```bash
    /usr/bin/python3 /opt/GuardianBridge/scripts/backup_guardianbridge.py
    ```

    The script creates timestamped backups in `/opt/GuardianBridge/backups` and prunes older backups (default retention: 14).
...
4.  **Optional: tune admin panel polling intervals** in `.env`:

    ```
    POLLING_INTERVAL_MS=5000
    CHAT_POLLING_INTERVAL_MS=2000
    ```

    Higher values reduce server load and bandwidth usage on slow links.
